...
export async function buildBackup(): Promise<BackupFile> {
  const [users, salons, customers, followups, activities, registrations, bookings, remarkOptions, settings] =
    await Promise.all([
      ...
      prisma.followup.findMany({ select: { customer: { select: { phone: true } }, nextFollowupDate: true, currentRemark: true, currentNote: true, lastContactedAt: true, lastContactedById: true, updatedById: true } }),
      prisma.activityLog.findMany({ select: { id: true, customer: { select: { phone: true } }, userId: true, activityType: true, remark: true, note: true, oldValue: true, newValue: true, createdAt: true } }),
