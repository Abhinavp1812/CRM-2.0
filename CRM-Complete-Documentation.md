...
**Exception — the Registered and Booked (type) tabs:** these two use a real sort instead of the capped page-1 pin, since the whole point of those tabs is browsing every recent one, not just a preview. A naive sort by `Customer.firstSeenAt` looks right but isn't: every customer created in the same sync batch shares the exact same `firstSeenAt`, since Postgres's `now()` is fixed for the whole transaction, not evaluated per row - within a single sync run, that ordering is arbitrary and can look unchanged from before. What "recent" actually means here is each customer's own date - `onboardingDate` on their `Registration` for Registered, their most recent `bookingDate` for Booked (type) - and Prisma can't sort a `findMany` by an aggregate of a to-many relation like that in one query (confirmed by a type error, not assumed).

Both tabs are instead driven by one small raw SQL query that resolves the correctly ordered, paginated list of customer ids directly - a `LEFT JOIN` against `Registration`/`Booking`, grouped by customer, ordered by `MAX(date) DESC NULLS LAST, "firstSeenAt" DESC`. `NULLS LAST` matters: Postgres's default for `DESC` is nulls *first*, which would otherwise put every customer without a date at the very top. That ordered id list then drives a normal Prisma fetch for the full row data, reordered in JS to match. Implementation: the `isRegistered || isBookedType` branch in `getTodayFollowups()`.
